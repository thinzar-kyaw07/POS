...
namespace POS.Sale
{
    public partial class frm_Sale : Form
    {
        ...
        private void frm_Sale_Load(object sender, EventArgs e)
        {
            ...
            SPString = String.Format("SP_Select_Sale N'{0}',N'{1}',N'{2}'", dtpDate.Value.ToShortDateString(), "0",
                   "1");
        ...
        private void dgvSale_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (ItemName != "")
            {
                if (CurCol - 1 == 1)
                {
                    ...
                    if (DT.Rows.Count <= 0)
                    {
                        MessageBox.Show("This ItemName Is Not Exist");
                        SendKeys.Send("{HOME}");
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < dgvSale.Rows.Count - 1; i++)
                        {
                            ...
                            {
                                MessageBox.Show("This Item Is Already Sale.");
                                AddRow = false;
                                SendKeys.Send("{HOME}");
                            }
                        }
                ...
                if (CurCol - 1 == 4)
                {
                    ...
                    if (int.TryParse(SaleQty, out OK) == false)
                    {
                     ...
                     else if (Convert.ToInt32(SaleQty) <= 0 || Convert.ToInt32(SaleQty) >
                     Convert.ToInt32(dgvSale.CurrentRow.Cells["QtyOnHand"].Value.ToString()))
                    {
                        MessageBox.Show("SaleQty Should Be Between 1 And " +
                        dgvSale.CurrentRow.Cells["QtyOnHand"].Value.ToString());
                        SendKeys.Send("{HOME}");
                        SendKeys.Send("{TAB}");
                    }
                    else
        ...
        private void txtPayment_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                ...
                if (e.KeyChar.Equals('\r'))
                {
                    ...
                    {
                        MessageBox.Show("Payment Amount Should Be Below " + (Convert.ToInt32(lblGrandTotal.Text)
                        + 10000));
                        txtPayment.Focus();
                        txtPayment.SelectAll();
                    }
   ...
   "2");
            ...
            {
                SPString = string.Format("SP_Select_Sale N'{0}',N'{1}',N'{2}'", dtpDate.Value.ToShortDateString(),
                "0", "1");
                lblSaleVoucher.Text = obj_clsMainDB.GetVoucher(SPString, dtpDate.Value.ToShortDateString());
            }
        }
