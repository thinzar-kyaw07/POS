...
namespace POS.MasterData
{
    public partial class frm_SupplierList : Form
    {
        ...
        private void tstSearchWith_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                SPString = string.Format("SP_Select_Supplier N'{0}' ,N'{1}' ,N'{2}'", tstSearchWith.Text.Trim().ToString().ToString(), "0", "2");
            }
            else if (tslLabel.Text == "Phone")
            {
                SPString = string.Format("SP_Select_Supplier N'{0}', N'{1}', N'{2}'", tstSearchWith.Text.Trim().ToString().ToString(), "0", "3");
            }
            else if (tslLabel.Text == "Address")
            {
                SPString = string.Format("SP_Select_Supplier N'{0}', N'{1}', N'{2}'", tstSearchWith.Text.Trim().ToString().ToString(), "0", "4");
            }
            dgvSupplier.DataSource = obj_clsMainDB.SelectData(SPString);
        }
